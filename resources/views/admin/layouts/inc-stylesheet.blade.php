<!-- plugins:css -->
<link rel="stylesheet" href="{{url('back/assets/vendors/iconfonts/simple-line-icon/css/simple-line-icons.css')}}">
<link rel="stylesheet" href="{{url('back/assets/vendors/iconfonts/flag-icon-css/css/flag-icon.min.css')}}">
<link rel="stylesheet" href="{{url('back/assets/vendors/css/vendor.bundle.base.css')}}">
<link rel="stylesheet" href="{{url('back/assets/vendors/css/vendor.bundle.addons.css')}}">
<!-- endinject -->
<!-- plugin css for this page -->
<link rel="stylesheet" href="{{url('back/assets/vendors/summernote/dist/summernote-bs4.css')}}">
<!-- End plugin css for this page -->
<link rel="stylesheet" href="{{url('back/assets/vendors/iconfonts/font-awesome/css/font-awesome.min.css')}}">
<!-- inject:css -->
<link rel="stylesheet" href="{{url('back/assets/css/style.css')}}">
<!-- endinject -->
<link rel="shortcut icon" href="{{url('back/assets/images/favicon.png')}}" />
